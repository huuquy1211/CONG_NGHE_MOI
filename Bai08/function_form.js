...
function displaySearchForm(res) {
    var data = fs.readFileSync(__dirname +'/Views/SearchForm.html', 'utf-8');
    res.writeHead(200, { 'Content-Type': 'text/html' });
    res.write(data);
}
function listTable(obj, res) {
    let tableHeader = '<table border="1px solid black"><tr><th>Year</th><th>Movie name</th><th>Movie type</th > <th>Author</th> <th>...</th></tr > ';
...
<td><a href="/edit?year=${book.year}&name=${book.name}&type=${book.type}&author=${book.
                    author}">Edit</a>
...
function addNewForm(res) {
    let data = fs.readFileSync(__dirname + '/Views/AddNewForm.html', 'utf-8');
    res.writeHead(200, { 'Content-Type': 'text/html' });
    res.write(data);
}
function editForm(year, name, type, author, res) {
    let data = fs.readFileSync(__dirname + '/Views/EditForm.html', 'utf-8');
    res.writeHead(200, { 'Content-Type': 'text/html' });
